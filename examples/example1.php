...
function scrapePage($url)
{
    ...
    if (!in_array($url, $scrapedPageUrls)) {
        ...
        $cdm = new CacheDownloadManager(new SimpleDownloadManager($url), dirname(dirname(__FILE__)) . "/var/cache/page/");
        $xpath = $cdm->getXpath();
...
function scrapeProduct($url)
{
    ...
    if (!in_array($url, $scrapedProductUrls)) {
        $cdm = new CacheDownloadManager(new SimpleDownloadManager($url), dirname(dirname(__FILE__)) . "/var/cache/product/");
        $dom = $cdm->getDom();
        $xpath = $cdm->getXpath();
        ...
        // Download image
        $im = new CacheDownloadManager(new SimpleDownloadManager($image), dirname(dirname(__FILE__)) . "/var/cache/image");
        $im->getContent();
